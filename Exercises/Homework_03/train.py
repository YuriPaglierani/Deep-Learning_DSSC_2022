...
def test_model(model, dataloader, performance=accuracy, loss_fn=None, device="cuda"):
    ...
    fin_loss_sum = loss_meter.sum if loss_fn is not None else None
    fin_loss_avg = loss_meter.avg if loss_fn is not None else None
    fin_perf_avg = performance_meter.avg
    print(f"TESTING - total loss: {round(fin_loss_sum, 0) if fin_loss_sum is not None else '--'}"+
          f" average loss: {round(fin_loss_avg, 3) if fin_loss_avg is not None else '--'}" +
          f"- performance {round(fin_perf_avg, 3) if fin_perf_avg is not None else '--'}\n")
    return fin_loss_sum, fin_loss_avg, fin_perf_avg